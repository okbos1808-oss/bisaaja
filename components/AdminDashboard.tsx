...
function KomentarPanel({
  ...
  useEffect(() => {
    ...
    async function fetchKomentar() {
      ...
      if (!active) return;
      setKomentar(data.map((d) => ({ ...d, createdAt: new Date(d.createdAt).toISOString() })));
      setLoading(false);
    }
    fetchKomentar();
    return () => { active = false; };
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full sm:w-105 bg-white z-50 flex flex-col shadow-2xl animate-slide-in">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="ml-auto flex gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setEditingId(k.id); setEditText(k.isi); }} className="p-1 rounded hover:bg-slate-200 text-slate-500 hover:text-blue-600 transition-colors" title="Edit">
                    ...
                    <button
                      onClick={() => {
                        ...
                        startTransition(async () => {
                          ...
                          if (res.success) setKomentar((prev) => prev.filter((item) => item.id !== k.id));
                        });
                      }}
                ...
                {editingId === k.id ? (
                  <div className="space-y-2">
                    <textarea value={editText} onChange={(e) => setEditText(e.target.value)} className="w-full border border-slate-300 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" rows={3} />
                    <div className="flex gap-2">
                      <button onClick={() => setEditingId(null)} className="px-3 py-1.5 bg-slate-200 hover:bg-slate-300 rounded-lg text-xs font-medium transition-colors">Batal</button>
                      <button
                        onClick={() => {
                          startTransition(async () => {
                            const res = await editKomentar(k.id, editText);
                            if (res.success) {
                              setKomentar((prev) => prev.map((item) => item.id === k.id ? { ...item, isi: editText } : item));
                              setEditingId(null);
                            }
                          });
                        }}
                  ...
                  <div className="bg-white p-3 rounded-xl rounded-tl-sm shadow-sm border border-slate-100 text-sm text-slate-700">{k.isi}</div>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="p-4 border-t bg-white">
          <div className="flex gap-2">
            <textarea value={text} onChange={(e) => setText(e.target.value)} onKeyDown={handleKeyDown} placeholder="Tulis komentar... (Enter untuk kirim)" className="flex-1 border border-slate-300 p-3 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" rows={2} />
            <button onClick={handleSend} disabled={!text.trim() || isPending} className="self-end px-4 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-xl transition-colors">
              {isPending ? (
                <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-4 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3">
        ...
        <button onClick={onOpenKomentar} className="shrink-0 w-9 h-9 rounded-full bg-blue-100 hover:bg-blue-200 flex items-center justify-center transition-colors">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          {item.fileDokumen ? (
            <a href={item.fileDokumen} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-blue-600 font-medium">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function AdminDashboard({ data }: { data: PermohonanType[] }) {
  ...
  const lastCountRef = useRef<number | null>(null); // ← null = belum diinit
  ...
  useEffect(() => {
    // Init lastCountRef dari API, bukan dari data prop
    async function init() {
      try {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        // Belum diinit, skip dulu
        if (lastCountRef.current === null) return;

        if (result.count > lastCountRef.current) {
          const jumlahBaru = result.count - lastCountRef.current;
          setNotifCount((prev) => prev + jumlahBaru);
          lastCountRef.current = result.count; // ← update ref, tidak trigger re-render
          router.refresh();
        }
      } catch (err) {
        console.error(err);
  ...
  }, [router]); // ← deps hanya router, interval tidak pernah di-reset
    ...
    <div className="flex items-center gap-3">
      {notifCount > 0 && (
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-full transition-colors"
        >
          🔔 {notifCount} baru
        </button>
      )}
        ...
        <div className="lg:hidden space-y-4">
          {filteredData.map((item) => (
            <MobileCard
              ...
              onOpenKomentar={() => setPanelData({ id: item.id, namaUsaha: item.namaUsaha, namaPemohon: item.user?.name ?? "Pemohon" })}
            />
          ))}
        </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4"><p className="font-medium text-slate-700">{item.namaUsaha}</p></td>
                      <td className="px-6 py-4">
                        <p className="font-medium text-slate-700">{item.layanan ?? "-"}</p>
                      ...
                      <td className="px-6 py-4">
                        {item.fileDokumen ? (
                          <a href={item.fileDokumen} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-xs font-medium transition-colors">
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <select
                            value={item.status}
                            disabled={loadingId === item.id}
                            onChange={(e) => handleStatusChange(item.id, e.target.value as StatusPermohonan)}
                             suppressHydrationWarning
                          ...
                          <button
                            onClick={() => setPanelData({ id: item.id, namaUsaha: item.namaUsaha, namaPemohon: item.user?.name ?? "Pemohon" })}
                             suppressHydrationWarning
